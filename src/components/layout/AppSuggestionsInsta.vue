...
import UiSuggestionInsta from "../ui/UiSuggestionInsta.vue";
export default {
  name: 'Insta',
  components: { UiSuggestionInsta },
  data: function () {
    return {
      users: [],
      swiper: null,
  ...
  computed: {
    chunkedArr() {
      const result = [];
      for (let i = 0; i < this.users.length; i += 30)
        result.push(this.users.slice(i, i + 30));
      return result;
    },
  },
  methods: {
